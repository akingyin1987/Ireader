...
import com.akingyin.quintic.libqpp.QppUsbCameraApi;
import com.akingyin.quintic.libqpp.iQppCallback;
...
public class BleQppNfcCameraServer   implements SDKInterface, BluetConnectListion,iQppCallback {
  ...
  public static BleQppNfcCameraServer getInstance(Context context) {
    ...
    if (null == bluetoothHelp) {
      bluetoothHelp = new BluetoothHelp(instance.context, address);
    }
    bluetoothHelp.setContext(instance.context);
  ...
  public void onregistered(RfidConnectorInterface rfidlistion) {
    ...
    String mac = KsiSharedStorageHelper.getBluetoothMac(KsiSharedStorageHelper.getPreferences(context));
    reConnectCount = 0;
  ...
  public void onSuccess(String bluemac) {
    ...
    if(TextUtils.isEmpty(bluemac)){
      String mac = KsiSharedStorageHelper
          .getBluetoothMac(KsiSharedStorageHelper.getPreferences(context));
  ...
  private   void   play(){
    pool.play(soundID, 0.8f, 0.8f, 1, 0, 1);
  }


  protected    Integer   getonElectricity(byte[]  results){

    String   data = ConvertUtils.bytesToHexString(results);
